...
class Ledger:
    ...
    def great(self, t):
        if t.sender == self.genesis_account.get_public_key(
        ):  # replace this with proper Genisis check
            return True
    ...
    def genesis(self, receiver, amount=GENESIS_AMOUNT):
        ...
        transaction = Transaction({}, self.genesis_account.get_public_key(),
                                  receiver, amount, [], None)
        transaction.valid = True
        self.genesis_account.sign(transaction)
        self.valid_transactions[transaction.sig] = transaction
        self.add_to_account_transactions(self.genesis_account.get_public_key(),
                                         transaction)
        self.add_to_account_transactions(receiver, transaction)

    def __str__(self):
        return "Open Transactions: %d - Valid Transactions: %d " % (
            len(list(self.open_transactions.keys())),
            len(list(self.valid_transactions.keys())))
