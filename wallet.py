...
class Wallet:
    ...
    def get_balance(self):
        return reduce(
            ...
            map(
                lambda t: -t.amount
                if t.sender == self.get_public_key() else t.amount,
                filter(
                    lambda t: t.sender == self.get_public_key() or self.ledger.
                    is_valid_transaction(t), self.my_transactions)), 0)
    ...
    def create_transaction(self, receiver, amount):
        ...
        transaction = Transaction(former.sig, self.get_public_key(), receiver,
                                  amount, reference.sig, None)
    ...
    def get_former(self, amount):
        for transaction in self.my_transactions:
            if transaction.receiver == self.get_public_key(
            ) and transaction.get_available_amount() >= amount:
                return transaction
        return None
