...
class Transaction:
    ...
    def get_amount_stacked(self):
        return reduce(
            ...
            map(lambda t: t.get_amount_stacked(), self.referenced_by),
            self.amount)
    ...
    def get_available_amount(self):
        return self.amount - reduce(lambda x, total: total + x,
                                    map(lambda t: t.amount, self.outputs), 0)
    ...
    def decerialize(transaction_string):
        values = list(
            map(lambda x: x.split(':')[1],
                transaction_string.split(SEPERATOR)))
        former = {}
        for f in list(
                map(lambda x: x.split('-'), values[0].split(ARR_SEPERATOR))):
            former[f[0]] = f[1]

        references = []
        for f in list(
                map(lambda x: x.split('-'), values[4].split(ARR_SEPERATOR))):
            references.append(f[0])

        return Transaction(former, values[1], values[2], int(values[3]),
                           references, values[5])

    def serialize_former(self):
        former = list(self.former.keys())
        return reduce(
            lambda former_string, f: f
            if former_string == "" else former_string + ARR_SEPERATOR + f,
            map(lambda f: f + "-" + self.former[f], former), "")

    def serialize_references(self):
        return reduce(
            lambda reference_string, r: r if reference_string == "" else
            reference_string + ARR_SEPERATOR + r, self.reference, "")

    def serialize(self, with_sig=True):
        former = self.serialize_former()
        references = self.serialize_references()
        transaction_string = "frm:%s%ssnd:%s%srcv:%s%samt:%d%sref:%s%s" % (
            former, SEPERATOR, self.sender, SEPERATOR, self.receiver,
            SEPERATOR, self.amount, SEPERATOR, references, SEPERATOR)
    ...
    def validate_sig(self):
        try:
            key = crypto_serialization.load_pem_public_key(
                self.sender.encode(), crypto_default_backend())
            key.verify(signature=bytes.fromhex(self.sig),
